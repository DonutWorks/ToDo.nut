class HistoriesController < ApplicationController
  ...
  def associate_history_with_images!
    ...
    params[:history][:images].each do |image|
      image_param = ActionController::Parameters.new({:image => image})
      history_image = HistoryImage.new(image_param.permit(:image))
      history_image.history_id = @history.id
      puts "saved image: #{history_image.save}"
    end
  end
end